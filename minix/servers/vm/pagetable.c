...
static int size = ARCH_VM_DIR_ENTRIES / 2; // 256
static int vm_self_pages;

/* PDE (Page Directory Entry) used to map in kernel, kernel physical address. */
...
/* Clicks must be pages, as     ////////////////////////click = one page
...
#endif
Queue* createQueue(pages)
{
	/* Create a Queue */
	Queue* Q;
	Q = (Queue*)malloc(sizeof(Queue));
	/* Initialise its properties */
	Q->elements = (int*)malloc(sizeof(int) * pages);
	Q->size = 0;
	Q->capacity = pages;
	Q->front = 0;
	Q->rear = -1;
...
static void fifo_add(int pages)  //add a page using fifo_add() which uses queues not linked lists.
{
	void Enqueue(Queue * Q, int pages)
	{
		...
		else
		{
			...
			/* Insert the pages in its rear side */
			Q->elements[Q->rear] = pages;
		}
		return;
	}
}
static void fifo_rm() //remove a page from the queue of the pages using fifo_rm()
{
	void Dequeue(Queue * Q)
	{
		...
		/* Removing a page is equivalent to incrementing index of front by one */
		else
		{
			Q->size--;
			Q->front++;
			/* As we fill pages in circular fashion */
	...
	int front(Queue * Q)
	{
		...
		/* Return the pages which is at the front*/
...
/* Basic pt sanity check. */
	...
	for(slot = 0; slot < ELEMENTS(vmproc); slot++) { //////we search inside vmprocess for a process connected to the page table
...
/* Find a space in the virtual address space of VM. */
	...
	curv = (u32_t) lastv; //3mlo initialization
	...
	while(curv < vmax) 
	{
		...
		if((pt->pt_dir[pde] & ARCH_VM_PDE_PRESENT) &&
		   (pt->pt_pt[pde][pte] & ARCH_VM_PTE_PRESENT)) {
			...
			/* there is no page here - so we have a hole, a bigger /// compaction for holes
			 * one if we already had one
			 */
			if(holev == NO_MEM) { // if it is the first hole 
				holev = curv;
				holesize = 1;
			}
			else holesize++; // if it not the first hole // holesize is the number of holes
...
{
	/*here we make sure that the virtual address length in bytes 
	is equal to or a multiple of the page size*/
	assert(!(vir % VM_PAGE_SIZE));  

	if(is_staticaddr(vir)) { /////we won't free if the vir bytes are less than the heap_start
...
/* Allocate a page for use by VM itself. */
	...
	void *ret;
	u32_t mem_flags = 0;
	fifo_add(pages); // bn-call el page replacement algorithm fifo
	//lru_add(pages); //bn-call el page replacement algorithm  lru
	assert(reason >= 0 && reason < VMP_CATEGORIES); // vmp_catgories = 4
 ...
 *				pt_ptalloc	     		     *
 *===========================================================================*/
static int pt_ptalloc(lvl2 *pt, int pde, u32_t flags)
{
	...
	assert(!pt->pt_pt[pde][0]);
	assert(!pt->pt_pt[pde][1]);
 ...
 *				pt_ptalloc1		     		     *
 *===========================================================================*/
static int pt_ptalloc1(pt_t *pt, int pde, u32_t flags)
{
/* Allocate a page table and write its address into the page directory. */
	int i;
	phys_bytes pt_phys;
...
/* Page directory and table entries for this virtual address. */
	...
	for(p = 0; p < pages; p++) {
		u32_t entry;
		int pde = ARCH_VM_PDE(v);  // pde = page directory entry
		int pte = ARCH_VM_PTE(v);  // pte = page table entry
 ...
 */
	...
	for(i = 0; i < ARCH_VM_DIR_ENTRIES; i++) { /////here we empty the directory and the page table slots
		pt->pt_dir[i] = 0; /* invalid entry (PRESENT bit = 0) */
		pt->pt_pt[i] = NULL;
	}
	///after emptying the page table we still point at it but with empty slots
...
int pt_new1(lvl2 * pt)
{
	...
	for (int i = 0; i < /*ARCH_VM_DIR_ENTRIES*/ size; i++) {
		pt->pt_dir[i] = 0;
		for (int j = 0; j < 2; j++)
		{
			pt->pt_dir[i] = 0; /* invalid entry (PRESENT bit = 0) */
...
void pt_init(void)
{
		...
		lvl2 *newpagetable;  // we declared an outerpage table
        int s, r, p;
	phys_bytes phys;
	 ...
	 */
	   ...
	   for (int i = 0; i < size; i++)
	   {
		   for (int j = 0; j < 2; j++)
		   {
			   newpagetable ->pt_pt[i][j] = pt_t->pt_pt[k]; // we initialize the outerpagetable
			   k++;
		   }
	   }
	...
	for(p = 0; p < ARCH_VM_DIR_ENTRIES; p++) {
		...
		if(pt_ptalloc(newpagetable, p, 0) != OK)
			panic("pt_ptalloc failed");
		if (pt_ptalloc1(newpt, p, 0) != OK)
			panic("pt_ptalloc1 failed");
		assert(newpt->pt_dir[p] & ARCH_VM_PDE_PRESENT);
		assert(newpagetable->pt_dir[p] & ARCH_VM_PDE_PRESENT);
	...
	pt_bind(newpagetable, &vmproc[VM_PROC_NR]);
	...
	pt_bind(newpagetable, &vmproc[VM_PROC_NR]);    /* Recalculate */
	pt_mapkernel(newpt);                    /* Rewrite pagetable info */////////////////////////
	pt_mapkernel(newpagetable); // maping kernel to the outer pagetable
 ...
 *				pt_bind			     		     *
 *===========================================================================*/
int pt_bind(lvl2* pt, struct vmproc* who)
{
 ...
 *				pt_bind1			     		     *
 *===========================================================================*/
int pt_bind1(pt_t *pt, struct vmproc *who)
{
	int procslot, pdeslot;
	u32_t phys;
	void *pdes;
